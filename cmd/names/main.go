...
func main() {
	...
	if discoveryURL != "" {
		id := n.(identity.Identity).ID()
		err := discovery.AdvertiseAndRegister(context.Background(), discovery.NewClient(discoveryURL, nil), id, advertiseAddr, actualPort, []string{"names-v1"}, tagFlags.Tags())
		if err != nil {
